...
static void update_reqs(struct pcapick *pp)
{
	...
	while (fgets(buf, sizeof buf, pp->gth)) {
		...
		/* ignore if too few columns */
		if (i < 7)
			continue;
		else
			pp->reqnum++;

		/* update minimum / maximum gt timestamps */
		if (pp->ts_start == 0 || req->start < pp->ts_start)
			pp->ts_start = req->start - 10.0;
		if (req->stop > pp->ts_stop)
			pp->ts_stop = req->stop + 10.0;

		/* is HTTPS? */
		https = (strncmp(req->appname, "https://", 8) == 0);

		/* find given IP */
		reqs = thash_get(pp->reqs, addr);
		if (!reqs) {
			reqs = mmatic_zalloc(pp->mm, sizeof *reqs);
			thash_set(pp->reqs, addr, reqs);

			reqs->http = tlist_create(NULL, pp->mm);
			reqs->https = tlist_create(NULL, pp->mm);
		}

		/* position list iterator on the first non-later web request */
		list = (https ? reqs->https : reqs->http);
		tlist_resetend(list);
		while ((req2 = tlist_iterback(list))) {
		...
		if (req2)
			tlist_insertafter(list, req);
		else
			tlist_prepend(list, req);
	...
	/* search for time reference points - remote IPs with the least number of requests
	 * NB: update when @1 is fixed */
	i = -1;
	thash_iter_loop(pp->reqs, addr, reqs) {
		j = tlist_count(reqs->https);

		if (j > 0 && (i == -1 || j < i)) {
			i = tlist_count(reqs->https);
			pp->min_addr = addr;

			tlist_reset(reqs->https);
			req = tlist_peek(reqs->https);
			pp->min_ts = req->start;
		}
	}
...
{
	struct req *req, *max_stop;
	struct reqs *reqs;
	tlist *list, *stack;

	update_reqs(pp);

	/* find web requests for this IP address */
	reqs = thash_get(pp->reqs, addr);
	if (!reqs) {
		pp->no_req_addr++;
		return NULL;
	...
	if (https) {
		...
		if (!reqs->https_stack) {
			if (tlist_count(list) == 0) {
				pp->no_req_addr++;
				return NULL;
			} else {
				reqs->https_stack = tlist_create(NULL, pp->mm);
			}
		}

		stack = reqs->https_stack;
	} else {
		list = reqs->http;
		...
		if (!reqs->http_stack) {
			if (tlist_count(list) == 0) {
				...
				reqs->http_stack = tlist_create(NULL, pp->mm);
			}
		}

		stack = reqs->http_stack;
	}

	/* cleanup the stack */
	if (tlist_count(stack) > 1) {
		tlist_reset(stack);
		max_stop = NULL;

		while ((req = tlist_iter(stack))) {
			if (ts > req->stop + PCAPICK_TIMEDIFF_DOWN) {
				tlist_remove(stack);

				/* remember the request with the highest stop time */
				if (!max_stop || req->stop > max_stop->stop)
					max_stop = req;
			}
		}

		/* promote the most recently finished request to fill any time gaps */
		if (tlist_count(stack) == 0 && max_stop)
			tlist_prepend(stack, max_stop);
	}

	/* cleanup the list */
	max_stop = NULL;
	tlist_reset(list);
	while ((req = tlist_iter(list))) {
		if (ts < req->start - PCAPICK_TIMEDIFF_DOWN) {
			break; /* @2 */
		} else if (ts > req->stop + PCAPICK_TIMEDIFF_UP) {
			tlist_remove(list);
			if (!max_stop || req->stop > max_stop->stop)
				max_stop = req;
		} else {
			tlist_remove(list);
			tlist_prepend(stack, req);
		}
	}

	/* handle border cases */
	if (tlist_count(stack) == 0) {
		if (max_stop) {
			tlist_prepend(stack, max_stop);
		} else if (req) {
			/* req != NULL means break @2:
			 * ts is too small for first req on list */
			pp->no_req_start++;
			return NULL;
		}
	}

	/* return first item on stack */
	tlist_reset(stack);
	req = tlist_peek(stack);

	if (ts > req->stop)
		req->stop = ts;

	if (!req) {
		dbg(5, "addr %s pkt ts=%.6f (+drift %.6f): request not found\n", addr, ts, ts+pp->drift);
	}
...
static bool tls_is_app(libtrace_packet_t *pkt)
{
	uint8_t proto;
	uint16_t ethertype;
	uint32_t rem;
	...
	double ts, bool up, bool is_new, libtrace_packet_t *pkt)
{
	 ...
	 */
	if (is_new) {
		/* check if we have chance to have ground truth data for it */
		if (pp->ts_start > 0 && (ts < pp->ts_start || ts > pp->ts_stop)) {
			f->ignore = true;
			return;
		}
...
found:
	 ...
	 */
	if (pp->min_ts > 0 && strcmp(pp->min_addr, f->addr) == 0 && tls_is_app(pkt)) {
		diff = pp->min_ts - ts;
		if (fabs(diff) < 900) {
			if (fabs(diff - pp->min_sugg) > 0.5) {
				printf("# time offset suggestion: %.6f\n", pp->drift + diff);
					pp->min_sugg = diff;
			}
		}
	}
...
//		uri = mmatic_sprintf(pp->mm, "pcapfile:%s/%s.pcap", dir, f->req->url);
		dir = mmatic_sprintf(pp->mm, "%s/%s", pp->dir, f->req->appname);
		uri = mmatic_sprintf(pp->mm, "pcapfile:%s/%s.pcap", dir, f->addr);
...
int main(int argc, char *argv[])
{
	...
	pp->mm = mm;
	pp->reqs = thash_create_strkey(NULL, mm); /* NB: no free fn */
	...
	printf("Matched: %8d packets\n", pp->with_req);
	printf("Dropped: %8d packets (addr %d / time %d)\n",
		pp->no_req, pp->no_req_addr, pp->no_req_start);
	printf("TOTAL:   %8d packets (%d requests)\n", pp->pktnum, pp->reqnum);
